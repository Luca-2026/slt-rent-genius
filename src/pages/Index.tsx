  ...
  return (
    <Layout>
      <SEO
        ...
        description="Baumaschinen, Anhänger & Event-Equipment mieten in NRW – Krefeld, Bonn & Mülheim. Über 1.700 Mietprodukte, Tiefpreisgarantie und Wochenendtarife."
        canonical="/"
        ...
        <div className="section-container">
           <AnimatedSection>
            <div className="max-w-4xl mx-auto prose prose-sm sm:prose-base text-muted-foreground px-1 sm:px-0">
              ...
              <p>
                {t("homeSeo.trailerIntro")} <button type="button" onClick={() => { setSeoLocationTarget("anhaenger"); setLocationDialogOpen(true); }} className="text-primary hover:underline font-medium inline cursor-pointer bg-transparent border-none p-0"><strong>{t("homeSeo.trailers")}</strong></button> {t("homeSeo.trailerText")}
              </p>
              <h3 className="text-base sm:text-lg font-semibold text-headline mt-6 sm:mt-8 mb-2 sm:mb-3">{t("homeSeo.priceHeading")}</h3>
              <p>
                {t("homeSeo.priceText1")} <strong>{t("homeSeo.priceGuarantee")}</strong>{t("homeSeo.priceText2")} <strong>{t("homeSeo.weekendRates")}</strong> {t("homeSeo.priceText3")} <Link to="/b2b" className="text-primary hover:underline font-medium">{t("homeSeo.b2bPortal")}</Link>.
              </p>
