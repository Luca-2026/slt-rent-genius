...
export default function CategoryProducts() {
  ...
  const categorySeoTexts: Record<string, string[]> = {
    ...
    "kabel-stromverteiler": [
      `Kabel und Stromverteiler mieten in ${location.name} – bei SLT Rental bekommst du das komplette Sortiment für Baustrom, Eventstrom und Notstrom aus einer Hand: CEE-Verlängerungskabel 16/32/63 A rot in Längen von 3, 5, 10, 20 und 50 m (Mennekes/Titanex H07RN-F), Schukokabel und Schuko-Kabeltrommel 50 m, Mennekes-Adapter (Schuko↔16 A, 16↔32 A, 32↔16 A, 32↔63 A, 63↔32 A inkl. Sicherung), CEE-Unterverteiler (16 A auf 3×/5× Schuko, 32 A auf 6× Schuko, 63 A auf 2× 32 A / 1× 16 A / 6× Schuko bzw. 19″-Rack-Variante), Defender-Kabelbrücken Micro 2 / Midi 5 / Office 1 m & 2 m sowie die Verteilerschränke Endverteilerschrank 22 kVA (EV32 Typ B) und 44 kVA (EV63 Typ B), Anschlussverteilerschrank 44 kVA und Hauptverteilerschrank 86 kVA (HV125 Typ B).`,
      `Konkrete Preisbeispiele: Der Endverteilerschrank EV32 mit 22 kVA / 32 A startet bei 22,61 €/Tag brutto und ist ideal für Baustellen-Unterverteilung sowie Event-Stromverteilung mit gemischtem 400 V/230 V-Bedarf. Der Anschlussverteilerschrank 44 kVA mit Zählerfeld ist ab 46,41 €/Tag brutto verfügbar und wird typischerweise als Übergabepunkt für den vom Netzbetreiber gesetzten Baustromzähler eingesetzt. Für Großbaustellen und Festivals stehen der EV63 (44 kVA) und der HV125 (86 kVA, Zuleitung über 125 A CEE oder 10 mm² Kabelschuhe) bereit. Alle Verteiler sind stückgeprüft, mit allstromsensitivem FI Typ B ausgestattet und damit auch für Frequenzumrichter, E-Ladegeräte und Wechselrichter freigegeben.`,
      `Wichtig zu wissen: Auf Baustellen schreibt die DGUV Information 203-006 („Auswahl und Betrieb elektrischer Anlagen und Betriebsmittel auf Bau- und Montagestellen") besondere Baustromverteiler nach DIN EN 61439-4 vor – Standard-Hausinstallationen oder einfache Steckdosenleisten sind dort nicht zulässig. Steckverbindungen über 16 A müssen 5-polig nach CEE (IEC 60309) ausgeführt sein. FI-Schutzschalter Typ B sind dann zwingend, wenn Verbraucher mit Frequenzumrichtern (z. B. moderne Bagger-Ladegeräte, Wechselrichter, Klimaaggregate) angeschlossen werden – ein reiner Typ A FI würde im Fehlerfall „erblinden". Genau deshalb setzen wir bei EV32, EV63 und HV125 ausschließlich auf FI Typ B.`,
      `Du brauchst keinen vollwertigen Hausanschluss, sondern willst nur temporären Baustrom oder Eventstrom in ${location.name}? Wir übernehmen den kompletten Baustromantrag beim zuständigen Netzbetreiber zum Festpreis, liefern den passenden Anschluss- oder Endverteilerschrank inklusive CEE-Zuleitung direkt zur Baustelle bzw. zum Veranstaltungsort und holen alles nach Mietende wieder ab. Faustformel für die Schrank-Auswahl: bis ca. 15 kW Verbraucherleistung reicht der EV32 (22 kVA), bis ca. 35 kW der EV63 (44 kVA), darüber empfehlen wir den HV125 (86 kVA) als Hauptverteiler mit nachgeschalteten Unterverteilern. Bei Fragen zur Lastauslegung, Kabelquerschnitt oder Phasenaufteilung beraten wir dich vor der Buchung kostenlos – per Mail oder telefonisch.`,
    ],
    beschallung: location.id === "krefeld" ? [
    ...
    nutzfahrzeuge: location.id === "krefeld" ? [
      ...
      `Mietkonditionen Bonn: 3,5t Pritschenwagen ab 65,45 €/Tag brutto, MAN TGL 7,5t ab 89,25 €/Tag brutto. Inklusive 100 Frei-Kilometer pro Tag (Mehrkilometer 0,23 € bzw. 0,48 € brutto), Vollkasko mit 2.500 € Selbstbeteiligung und 1.000 € Kaution. Kraftstoff (Diesel, beim MAN zusätzlich AdBlue) wird bei nicht vollgetankter Rückgabe nachberechnet, Maut beim 7,5-Tonner per Einzelfahrtnachweis. Die Fahrzeuge sind selbstfahrend und werden direkt am Standort Bonn übergeben – keine Tieflader-Anlieferung nötig.`,
    ] : [
  ...
  const categoryFaqs: Record<string, { question: string; answer: string }[]> = {
    ...
    "kabel-stromverteiler": [
      { question: `Was kostet ein Stromverteiler mieten in ${location.name}?`, answer: `Die Tagesmieten starten bei kleinen CEE-Unterverteilern und Verlängerungskabeln im einstelligen Euro-Bereich. Der Endverteilerschrank EV32 mit 22 kVA / 32 A ist ab 22,61 €/Tag brutto verfügbar, der Anschlussverteilerschrank 44 kVA mit Zählerfeld ab 46,41 €/Tag brutto. Für den großen Hauptverteilerschrank HV125 (86 kVA) erstellen wir ein individuelles Angebot. Bei mehrtägiger oder wöchentlicher Miete reduziert sich der Tagessatz – den finalen Preis siehst du transparent im Buchungsprozess.` },
      { question: `Wann brauche ich einen Baustromverteiler statt einer normalen Steckdose?`, answer: `Auf Baustellen schreibt die DGUV Information 203-006 Baustromverteiler nach DIN EN 61439-4 vor – eine reguläre Hausinstallation ist dort nicht zulässig. Steckverbindungen über 16 A müssen 5-polig nach CEE (IEC 60309) ausgeführt sein. Sobald mehrere Verbraucher gleichzeitig betrieben werden, Maschinen mit 32 A oder 63 A CEE-Stecker im Einsatz sind oder Verbraucher mit Frequenzumrichtern (E-Ladegeräte, Wechselrichter, moderne Klimaaggregate) angeschlossen werden, ist ein Verteiler mit allstromsensitivem FI Typ B Pflicht.` },
      { question: `Welche Größe Baustromverteiler brauche ich?`, answer: `Faustformel: bis ca. 15 kW Verbraucherleistung (kleinere Bauvorhaben, einzelne Maschinen, Eventstände) reicht der EV32 mit 22 kVA. Bis ca. 35 kW – Kräne, Mischanlagen, Bautrockner, mehrere parallele Verbraucher – empfehlen wir den EV63 mit 44 kVA. Großbaustellen und Festivals mit zentraler Hauptverteilung und mehreren Unterverteilern fahren mit dem HV125 (86 kVA) am besten. Wir beraten dich gern bei der konkreten Lastauslegung.` },
      { question: `Was ist der Unterschied zwischen FI Typ A und FI Typ B?`, answer: `Ein FI Typ A erkennt sinusförmige Wechselfehlerströme und pulsierende Gleichfehlerströme – ausreichend für Standard-Verbraucher. FI Typ B (allstromsensitiv) erkennt zusätzlich glatte Gleichfehlerströme. Verbraucher mit Frequenzumrichtern (z. B. E-Ladegeräte, Wechselrichter, moderne Klimaanlagen, drehzahlgeregelte Antriebe) können einen Typ A FI „blind" machen. EV32, EV63 und HV125 sind deshalb durchgängig mit FI Typ B ausgestattet.` },
      { question: `Welche CEE-Kabellängen und Adapter habt ihr im Sortiment?`, answer: `Wir führen CEE-Verlängerungskabel in 16, 32 und 63 A rot in den Längen 3, 5, 10, 20 und 50 m (Mennekes/Titanex H07RN-F), Schukokabel und Schuko-Kabeltrommel 50 m sowie die gängigen Mennekes-Adapter: Schuko↔CEE 16 A, CEE 16↔32 A, 32↔16 A, 32↔63 A und 63↔32 A inklusive Sicherung. Damit lassen sich nahezu alle Verbraucher und Anschlüsse miteinander kombinieren.` },
      { question: `Übernehmt ihr auch die Beantragung des Baustromanschlusses?`, answer: `Ja. Wir übernehmen die komplette Beantragung des Baustromanschlusses beim zuständigen Netzbetreiber zum Festpreis und liefern den passenden Anschlussverteilerschrank (44 kVA mit Zählerfeld) bzw. Endverteilerschrank inklusive CEE-Zuleitung direkt zur Baustelle in ${location.name} und Umgebung. So sparst du dir den Behördengang und hast einen Ansprechpartner für Strom, Verteiler, Kabel und Antrag.` },
      { question: `Welche Kabelbrücken sind für Veranstaltungen geeignet?`, answer: `Für die sichere Kabelführung führen wir Defender Micro 2 (2 Kanäle) und Defender Midi 5 (5 Kanäle, max. Achslast 5 t, Brandschutzklasse B2) für den Outdoor- und Befahrbar-Einsatz sowie Office-Kabelbrücken in 1 m und 2 m Länge mit 4 Kanälen für die Innenanwendung – wichtig für die Stolperprävention nach DGUV V3 / ArbStättV bei Events.` },
    ],
    beschallung: location.id === "krefeld" ? [
    ...
    nutzfahrzeuge: [
      { question: `Welche Nutzfahrzeuge kann ich bei SLT Rental in ${location.name} mieten?`, answer: `Am Standort ${location.name} stehen zwei selbstfahrende Dreiseitenkipper auf Anfrage zur Verfügung: der 3,5t Pritschenwagen mit Doppelkabine (Opel Movano oder vergleichbar, 7 Sitze, 1.135 kg Nutzlast, AHK 2,5 t) ab 65,45 €/Tag brutto und der 7,5t MAN TGL 8.220 Dreiseitenkipper mit Meiller D205 Aufbau (220 PS, Automatik, Euro 6e) ab 89,25 €/Tag brutto.` },
      ...
      { question: `Wie sind Kraftstoff, Frei-Kilometer und Maut geregelt?`, answer: `Im Mietpreis sind 100 Frei-Kilometer pro Tag enthalten. Beim 3,5-Tonner werden Mehrkilometer mit 0,27 € brutto berechnet, beim MAN TGL mit 0,48 € brutto. Die Rückgabe erfolgt vollgetankt; nicht aufgefüllter Diesel wird mit 2,85 €/l (brutto bzw. netto je Fahrzeug) und beim MAN AdBlue mit 2,20 €/l brutto. nachberechnet. Für den 7,5t MAN TGL gilt seit dem 01.07.2024 die LKW-Maut – wir berechnen sie per Einzelfahrtnachweis weiter.` },
  ...
  const expertContent = getCategoryExpertContent(location.id, category.id);
  const faqs = expertContent?.faqs || moebelZelteCustom?.faqs || categoryFaqs[category.id] || [
