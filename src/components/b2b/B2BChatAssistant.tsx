...
const CHAT_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/b2b-chat`;

const SUGGESTED_QUESTIONS = [
  "Welche Mietvorgänge sind aktuell aktiv?",
  "Habe ich offene Rechnungen?",
  "Wie hoch ist mein Kreditlimit?",
  "Wie kann ich ein Gerät zur Abholung freimelden?",
];

export function B2BChatAssistant() {
  ...
  useEffect(() => {
    if (open && messages.length === 0) {
      setMessages([{
        role: "assistant",
        content: "Hallo! Ich bin dein persönlicher KI-Assistent. Ich habe Zugriff auf deine Mietvorgänge, Rechnungen und Kontodaten. Wie kann ich dir helfen?",
      }]);
    }
  }, [open]);
      ...
      {open && (
        <div className="fixed bottom-24 left-6 z-50 w-[360px] max-w-[calc(100vw-1.5rem)] bg-background border border-border rounded-2xl shadow-2xl flex flex-col overflow-hidden"
          ...
          <div className="bg-primary px-4 py-3 flex items-center gap-3">
            ...
            <div>
              <p className="text-primary-foreground font-semibold text-sm">KI-Assistent</p>
              <p className="text-primary-foreground/70 text-xs">Fragen zu Miete, Rechnungen & Konto</p>
            </div>
          </div>
