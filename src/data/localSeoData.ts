// Local SEO data for cities within 15km radius of each SLT location

export interface LocalArea {
  slug: string;
  name: string;
  region: string;
  locationId: string; // nearest SLT location
  distance: number; // km from SLT location
  population?: number;
  postalCodes?: string[];
  description: string;
  keywords: string[];
}

export const localAreas: LocalArea[] = [
  // === KREFELD (47807) - 15km radius ===
  {
    slug: "krefeld",
    name: "Krefeld",
    region: "Niederrhein",
    locationId: "krefeld",
    distance: 0,
    population: 227000,
    postalCodes: ["47798", "47799", "47800", "47802", "47803", "47804", "47805", "47807", "47809", "47829"],
    description: "Mieten Sie Baumaschinen, Anhänger und Equipment direkt in Krefeld. SLT Rental ist Ihr lokaler Partner für professionelle Mietgeräte am Niederrhein.",
    keywords: ["Baumaschinen mieten Krefeld", "Anhänger mieten Krefeld", "Bagger mieten Krefeld", "Minibagger Krefeld"],
  },
  {
    slug: "meerbusch",
    name: "Meerbusch",
    region: "Rhein-Kreis Neuss",
    locationId: "krefeld",
    distance: 8,
    population: 57000,
    postalCodes: ["40667", "40668", "40670"],
    description: "Baumaschinen und Anhänger mieten in Meerbusch. Schnelle Abholung bei SLT Rental in Krefeld-Fichtenhain – nur 8 km entfernt.",
    keywords: ["Baumaschinen mieten Meerbusch", "Anhänger mieten Meerbusch", "Mietgeräte Meerbusch"],
  },
  {
    slug: "willich",
    name: "Willich",
    region: "Kreis Viersen",
    locationId: "krefeld",
    distance: 7,
    population: 51000,
    postalCodes: ["47877"],
    description: "Equipment mieten in Willich. SLT Rental in Krefeld bietet Ihnen Minibagger, Anhänger und Werkzeuge zur Miete – nur 7 km entfernt.",
    keywords: ["Baumaschinen mieten Willich", "Anhänger mieten Willich", "Minibagger Willich"],
  },
  {
    slug: "toenisvorst",
    name: "Tönisvorst",
    region: "Kreis Viersen",
    locationId: "krefeld",
    distance: 6,
    population: 30000,
    postalCodes: ["47918"],
    description: "Mietgeräte in Tönisvorst. Von Bagger bis Anhänger – SLT Rental Krefeld ist nur 6 km entfernt und bietet faire Mietpreise.",
    keywords: ["Baumaschinen mieten Tönisvorst", "Anhänger mieten Tönisvorst", "Bagger mieten Tönisvorst"],
  },
  {
    slug: "kempen",
    name: "Kempen",
    region: "Kreis Viersen",
    locationId: "krefeld",
    distance: 12,
    population: 35000,
    postalCodes: ["47906"],
    description: "Baumaschinen mieten in Kempen. SLT Rental in Krefeld-Fichtenhain ist nur 12 km entfernt – mit großer Auswahl an Mietgeräten.",
    keywords: ["Baumaschinen mieten Kempen", "Anhänger mieten Kempen", "Mietgeräte Kempen"],
  },
  {
    slug: "moers",
    name: "Moers",
    region: "Kreis Wesel",
    locationId: "krefeld",
    distance: 14,
    population: 104000,
    postalCodes: ["47441", "47443", "47445", "47447"],
    description: "Equipment mieten in Moers. Bei SLT Rental Krefeld finden Sie Minibagger, Anhänger und Baugeräte zur Miete – nur 14 km entfernt.",
    keywords: ["Baumaschinen mieten Moers", "Anhänger mieten Moers", "Minibagger mieten Moers"],
  },
  {
    slug: "duisburg-west",
    name: "Duisburg (West)",
    region: "Ruhrgebiet",
    locationId: "krefeld",
    distance: 12,
    population: 500000,
    postalCodes: ["47137", "47138", "47139", "47166", "47167", "47169"],
    description: "Baumaschinen und Anhänger mieten in Duisburg-West. SLT Rental Krefeld bedient den westlichen Raum Duisburg – nur 12 km Entfernung.",
    keywords: ["Baumaschinen mieten Duisburg", "Anhänger mieten Duisburg", "Bagger mieten Duisburg West"],
  },
  {
    slug: "neuss",
    name: "Neuss",
    region: "Rhein-Kreis Neuss",
    locationId: "krefeld",
    distance: 15,
    population: 153000,
    postalCodes: ["41460", "41462", "41464", "41466", "41468", "41469", "41470", "41472"],
    description: "Mietgeräte in Neuss. SLT Rental Krefeld bietet Baumaschinen, Anhänger und Werkzeuge – nur 15 km von Neuss entfernt.",
    keywords: ["Baumaschinen mieten Neuss", "Anhänger mieten Neuss", "Minibagger Neuss"],
  },
  {
    slug: "viersen",
    name: "Viersen",
    region: "Kreis Viersen",
    locationId: "krefeld",
    distance: 13,
    population: 77000,
    postalCodes: ["41747", "41748", "41749", "41751"],
    description: "Equipment mieten in Viersen. Bei SLT Rental Krefeld finden Sie professionelle Mietgeräte – nur 13 km entfernt.",
    keywords: ["Baumaschinen mieten Viersen", "Anhänger mieten Viersen", "Mietgeräte Viersen"],
  },
  {
    slug: "kaarst",
    name: "Kaarst",
    region: "Rhein-Kreis Neuss",
    locationId: "krefeld",
    distance: 10,
    population: 44000,
    postalCodes: ["41564"],
    description: "Baumaschinen mieten in Kaarst. SLT Rental Krefeld ist nur 10 km entfernt und bietet Anhänger, Bagger und mehr.",
    keywords: ["Baumaschinen mieten Kaarst", "Anhänger mieten Kaarst", "Bagger Kaarst"],
  },

  // === BONN (53179) - 15km radius ===
  {
    slug: "bonn",
    name: "Bonn",
    region: "Rheinland",
    locationId: "bonn",
    distance: 0,
    population: 331000,
    postalCodes: ["53111", "53113", "53115", "53117", "53119", "53121", "53123", "53125", "53127", "53129", "53173", "53175", "53177", "53179"],
    description: "Mieten Sie Baumaschinen, Anhänger und Equipment direkt in Bonn. SLT Rental ist Ihr lokaler Partner für professionelle Mietgeräte im Rheinland.",
    keywords: ["Baumaschinen mieten Bonn", "Anhänger mieten Bonn", "Bagger mieten Bonn", "Minibagger Bonn"],
  },
  {
    slug: "bad-godesberg",
    name: "Bad Godesberg",
    region: "Bonn",
    locationId: "bonn",
    distance: 3,
    population: 75000,
    postalCodes: ["53173", "53175", "53177", "53179"],
    description: "Mietgeräte in Bad Godesberg. SLT Rental Bonn bietet Baumaschinen und Anhänger – direkt vor Ort in der Region.",
    keywords: ["Baumaschinen mieten Bad Godesberg", "Anhänger mieten Bad Godesberg", "Mietgeräte Bad Godesberg"],
  },
  {
    slug: "koenigswinter",
    name: "Königswinter",
    region: "Rhein-Sieg-Kreis",
    locationId: "bonn",
    distance: 10,
    population: 42000,
    postalCodes: ["53639"],
    description: "Equipment mieten in Königswinter. Bei SLT Rental Bonn finden Sie Minibagger, Anhänger und mehr – nur 10 km entfernt.",
    keywords: ["Baumaschinen mieten Königswinter", "Anhänger mieten Königswinter", "Bagger Königswinter"],
  },
  {
    slug: "bad-honnef",
    name: "Bad Honnef",
    region: "Rhein-Sieg-Kreis",
    locationId: "bonn",
    distance: 14,
    population: 26000,
    postalCodes: ["53604"],
    description: "Baumaschinen mieten in Bad Honnef. SLT Rental Bonn ist nur 14 km entfernt und bietet professionelle Mietgeräte.",
    keywords: ["Baumaschinen mieten Bad Honnef", "Anhänger mieten Bad Honnef", "Mietgeräte Bad Honnef"],
  },
  {
    slug: "sankt-augustin",
    name: "Sankt Augustin",
    region: "Rhein-Sieg-Kreis",
    locationId: "bonn",
    distance: 8,
    population: 56000,
    postalCodes: ["53757"],
    description: "Mietgeräte in Sankt Augustin. SLT Rental Bonn bietet Baumaschinen, Anhänger und Werkzeuge – nur 8 km entfernt.",
    keywords: ["Baumaschinen mieten Sankt Augustin", "Anhänger mieten Sankt Augustin", "Minibagger Sankt Augustin"],
  },
  {
    slug: "siegburg",
    name: "Siegburg",
    region: "Rhein-Sieg-Kreis",
    locationId: "bonn",
    distance: 12,
    population: 42000,
    postalCodes: ["53721"],
    description: "Equipment mieten in Siegburg. Bei SLT Rental Bonn finden Sie professionelle Mietgeräte – nur 12 km von Siegburg entfernt.",
    keywords: ["Baumaschinen mieten Siegburg", "Anhänger mieten Siegburg", "Bagger mieten Siegburg"],
  },
  {
    slug: "troisdorf",
    name: "Troisdorf",
    region: "Rhein-Sieg-Kreis",
    locationId: "bonn",
    distance: 14,
    population: 76000,
    postalCodes: ["53840", "53842", "53844"],
    description: "Baumaschinen und Anhänger mieten in Troisdorf. SLT Rental Bonn bedient Troisdorf mit fairen Mietpreisen – nur 14 km entfernt.",
    keywords: ["Baumaschinen mieten Troisdorf", "Anhänger mieten Troisdorf", "Mietgeräte Troisdorf"],
  },
  {
    slug: "alfter",
    name: "Alfter",
    region: "Rhein-Sieg-Kreis",
    locationId: "bonn",
    distance: 7,
    population: 24000,
    postalCodes: ["53347"],
    description: "Mietgeräte in Alfter. SLT Rental Bonn ist Ihr Partner für Baumaschinen und Anhänger – nur 7 km entfernt.",
    keywords: ["Baumaschinen mieten Alfter", "Anhänger mieten Alfter", "Bagger Alfter"],
  },
  {
    slug: "bornheim",
    name: "Bornheim",
    region: "Rhein-Sieg-Kreis",
    locationId: "bonn",
    distance: 12,
    population: 48000,
    postalCodes: ["53332"],
    description: "Equipment mieten in Bornheim. Bei SLT Rental Bonn finden Sie Minibagger, Anhänger und mehr – nur 12 km entfernt.",
    keywords: ["Baumaschinen mieten Bornheim", "Anhänger mieten Bornheim", "Minibagger Bornheim"],
  },
  {
    slug: "meckenheim",
    name: "Meckenheim",
    region: "Rhein-Sieg-Kreis",
    locationId: "bonn",
    distance: 13,
    population: 25000,
    postalCodes: ["53340"],
    description: "Baumaschinen mieten in Meckenheim. SLT Rental Bonn bietet professionelle Mietgeräte – nur 13 km entfernt.",
    keywords: ["Baumaschinen mieten Meckenheim", "Anhänger mieten Meckenheim", "Mietgeräte Meckenheim"],
  },
  {
    slug: "rheinbach",
    name: "Rheinbach",
    region: "Rhein-Sieg-Kreis",
    locationId: "bonn",
    distance: 15,
    population: 27000,
    postalCodes: ["53359"],
    description: "Mietgeräte in Rheinbach. SLT Rental Bonn ist Ihr Partner für Baumaschinen und Anhänger – nur 15 km entfernt.",
    keywords: ["Baumaschinen mieten Rheinbach", "Anhänger mieten Rheinbach", "Bagger Rheinbach"],
  },

  // === MÜLHEIM AN DER RUHR (45478) - 15km radius ===
  {
    slug: "muelheim-an-der-ruhr",
    name: "Mülheim an der Ruhr",
    region: "Ruhrgebiet",
    locationId: "muelheim",
    distance: 0,
    population: 171000,
    postalCodes: ["45468", "45470", "45472", "45473", "45475", "45476", "45478", "45479", "45481"],
    description: "Mieten Sie Baumaschinen und Anhänger direkt in Mülheim an der Ruhr. SLT Rental ist Ihr lokaler Partner im Ruhrgebiet.",
    keywords: ["Baumaschinen mieten Mülheim", "Anhänger mieten Mülheim", "Bagger mieten Mülheim an der Ruhr"],
  },
  {
    slug: "essen",
    name: "Essen",
    region: "Ruhrgebiet",
    locationId: "muelheim",
    distance: 8,
    population: 583000,
    postalCodes: ["45127", "45128", "45130", "45131", "45133", "45134", "45136", "45138", "45139", "45141", "45143", "45144", "45145", "45147", "45149"],
    description: "Baumaschinen und Anhänger mieten in Essen. SLT Rental Mülheim bedient ganz Essen – nur 8 km vom Standort entfernt.",
    keywords: ["Baumaschinen mieten Essen", "Anhänger mieten Essen", "Minibagger Essen", "Bagger mieten Essen"],
  },
  {
    slug: "oberhausen",
    name: "Oberhausen",
    region: "Ruhrgebiet",
    locationId: "muelheim",
    distance: 6,
    population: 210000,
    postalCodes: ["46045", "46047", "46049", "46117", "46119", "46145", "46147", "46149"],
    description: "Mietgeräte in Oberhausen. SLT Rental Mülheim bietet Baumaschinen und Anhänger – nur 6 km von Oberhausen entfernt.",
    keywords: ["Baumaschinen mieten Oberhausen", "Anhänger mieten Oberhausen", "Bagger Oberhausen"],
  },
  {
    slug: "duisburg-sued",
    name: "Duisburg (Süd)",
    region: "Ruhrgebiet",
    locationId: "muelheim",
    distance: 10,
    population: 500000,
    postalCodes: ["47051", "47053", "47055", "47057", "47058", "47059"],
    description: "Equipment mieten in Duisburg-Süd. Bei SLT Rental Mülheim finden Sie professionelle Mietgeräte – nur 10 km entfernt.",
    keywords: ["Baumaschinen mieten Duisburg Süd", "Anhänger mieten Duisburg", "Mietgeräte Duisburg"],
  },
  {
    slug: "bottrop",
    name: "Bottrop",
    region: "Ruhrgebiet",
    locationId: "muelheim",
    distance: 12,
    population: 117000,
    postalCodes: ["46236", "46238", "46240", "46242", "46244"],
    description: "Baumaschinen mieten in Bottrop. SLT Rental Mülheim ist nur 12 km entfernt und bietet Anhänger, Bagger und mehr.",
    keywords: ["Baumaschinen mieten Bottrop", "Anhänger mieten Bottrop", "Minibagger Bottrop"],
  },
  {
    slug: "gelsenkirchen",
    name: "Gelsenkirchen",
    region: "Ruhrgebiet",
    locationId: "muelheim",
    distance: 14,
    population: 260000,
    postalCodes: ["45879", "45881", "45883", "45884", "45886", "45888", "45889", "45891", "45892", "45894", "45896", "45897", "45899"],
    description: "Mietgeräte in Gelsenkirchen. SLT Rental Mülheim bietet Baumaschinen und Anhänger – nur 14 km von Gelsenkirchen entfernt.",
    keywords: ["Baumaschinen mieten Gelsenkirchen", "Anhänger mieten Gelsenkirchen", "Bagger Gelsenkirchen"],
  },
  {
    slug: "ratingen",
    name: "Ratingen",
    region: "Kreis Mettmann",
    locationId: "muelheim",
    distance: 15,
    population: 87000,
    postalCodes: ["40878", "40880", "40882", "40883", "40885"],
    description: "Equipment mieten in Ratingen. Bei SLT Rental Mülheim finden Sie professionelle Mietgeräte – nur 15 km entfernt.",
    keywords: ["Baumaschinen mieten Ratingen", "Anhänger mieten Ratingen", "Mietgeräte Ratingen"],
  },
  {
    slug: "bochum-west",
    name: "Bochum (West)",
    region: "Ruhrgebiet",
    locationId: "muelheim",
    distance: 15,
    population: 365000,
    postalCodes: ["44787", "44789", "44791", "44793", "44795", "44797", "44799"],
    description: "Baumaschinen und Anhänger mieten in Bochum-West. SLT Rental Mülheim bedient den westlichen Raum Bochum.",
    keywords: ["Baumaschinen mieten Bochum", "Anhänger mieten Bochum West", "Bagger Bochum"],
  },
  {
    slug: "dinslaken",
    name: "Dinslaken",
    region: "Kreis Wesel",
    locationId: "muelheim",
    distance: 14,
    population: 70000,
    postalCodes: ["46535", "46537", "46539"],
    description: "Mietgeräte in Dinslaken. SLT Rental Mülheim ist Ihr Partner für Baumaschinen und Anhänger – nur 14 km entfernt.",
    keywords: ["Baumaschinen mieten Dinslaken", "Anhänger mieten Dinslaken", "Minibagger Dinslaken"],
  },
];

// Helper functions
export function getAreasForLocation(locationId: string): LocalArea[] {
  return localAreas.filter((area) => area.locationId === locationId);
}

export function getAreaBySlug(slug: string): LocalArea | undefined {
  return localAreas.find((area) => area.slug === slug);
}

export function getAllAreaSlugs(): string[] {
  return localAreas.map((area) => area.slug);
}
